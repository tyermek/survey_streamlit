...
def plot_responses(df, plot_types):
    for plot_type in plot_types:
        if plot_type == "Үйде жұмыс орнының қолжетімділігі":
            ...
            fig1.update_layout(xaxis_tickangle=-45, width=900, height=500, legend=dict(x=1, y=0.5))
            ...
            st.write(table_data1.set_index('Жауап').T)
            ...
            st.write(table_data2.set_index(['16. Компьютерде күніне қанша сағат отырасыз?', '18. Қашықтықтан оқыту кезінде гимнастикалық жаттығуды Сіз күніне неше рет жасайсыз?']).T)
            ...
            fig3.update_layout(xaxis_tickangle=-45, width=900, height=500, legend=dict(x=1, y=0.5))
            ...
            st.write(table_data3.set_index('Жауап').T)
            ...
            st.write(table_data4.set_index(['15. Үй жағдайында Сізге онлайн сабақтарға қатысу ыңғайлы ма?', '22. Сізге қалай сабақ оқу ыңғайлы?']).T)
            ...
            fig.update_layout(xaxis_tickangle=-45, width=900, height=500, legend=dict(x=1, y=0.5))
            st.plotly_chart(fig, use_container_width=True)
...
# Multiselect for selecting the type of plots with a placeholder
