...
def plot_responses(df, plot_types):
    for plot_type in plot_types:
        if plot_type == "Үйде жұмыс орнының қолжетімділігі":
            ...
            fig1.update_layout(xaxis_tickangle=-45, width=800, height=600, margin=dict(l=0, r=0, t=30, b=150), font=dict(size=14))
            ...
            fig2 = px.bar(df,
                          x="16.\тКомпьютерде күніне қанша сағат отырасыз? / Сколько часов в день Вы сидите за компьютером?",
                          color="18.\тҚашықтықтан оқыту кезінде гимнастикалық жаттығуды Сіз күніне неше рет жасайсыз? / Сколько раз в день Вы выполняете гимнастическую разминку при дистанционном обучении?",
                          barmode='group',
                          title="Қашықтықтан оқыту кезінде гимнастикалық жаттығуды Сіз күніне неше рет жасайсыз?")
            fig2.update_layout(xaxis_tickangle=-45, width=800, height=600, margin=dict(l=0, r=0, t=30, b=150), font=dict(size=14))
            st.plotly_chart(fig2)
            table_data2 = df.groupby(["16.\тКомпьютерде күніне қанша сағат отырасыз? / Сколько часов в день Вы сидите за компьютером?", "18.\тҚашықтықтан оқыту кезінде гимнастикалық жаттығуды Сіз күніне неше рет жасайсыз? / Сколько раз в день Вы выполняете гимнастическую разминку при дистанционном обучении?"]).size().reset_index(name='Саны')
            st.write(table_data2)
        elif plot_type == "Сізге онлайн сабақтарға қатысу ыңғайлы ма?":
            fig = px.bar(df,
                         x="15.\тҮй жағдайында Сізге онлайн сабақтарға қатысу ыңғайлы ма? / Удобно ли Вам в домашних условиях участвовать в онлайн уроках? ",
                         title="Сізге онлайн сабақтарға қатысу ыңғайлы ма?")
            fig.update_layout(xaxis_tickangle=-45, width=800, height=600, margin=dict(l=0, r=0, t=30, b=150), font=dict(size=14))
            st.plotly_chart(fig)
            table_data = df["15.\тҮй жағдайында Сізге онлайн сабақтарға қатысу ыңғайлы ма? / Удобно ли Вам в домашних условиях участвовать в онлайн уроках? "].value_counts().reset_index()
            table_data.columns = ["Жауап", "Саны"]
            st.write(table_data)
        elif plot_type == "Компьютер немесе гаджет салдарынан отбасы мүшелерімен жанжал туындайды ма?":
            fig = px.pie(df,
                         names="27.\тКомпьютер немесе гаджет салдарынан отбасы мүшелерімен жанжал туындайды ма? / Возникают ли ссоры с членами семьи из-за компьютера или гаджета?",
                         title="Компьютер немесе гаджет салдарынан отбасы мүшелерімен жанжал туындайды ма?")
            fig.update_layout(width=800, height=600)
            st.plotly_chart(fig)
            table_data = df["27.\тКомпьютер немесе гаджет салдарынан отбасы мүшелерімен жанжал туындайды ма? / Возникают ли ссоры с членами семьи из-за компьютера или гаджета?"].value_counts().reset_index()
            table_data.columns = ["Жауап", "Саны"]
            st.write(table_data)
        elif plot_type == "Мұғалімнің тапсырмаларын қалай жиі орындайсыз?":
            fig = px.pie(df,
                         names="10.\тМұғалімнің тапсырмаларын қалай жиі орындайсыз? / Как часто Вы выполняете задания учителя?",
                         title="Мұғалімнің тапсырмаларын қалай жиі орындайсыз?")
            fig.update_layout(width=800, height=600)
            st.plotly_chart(fig)
            table_data = df["10.\тМұғалімнің тапсырмаларын қалай жиі орындайсыз? / Как часто Вы выполняете задания учителя?"].value_counts().reset_index()
            table_data.columns = ["Жауап", "Саны"]
            st.write(table_data)
        elif plot_type == "Сіз қашықтықтан оқыту кезінде белсенді бола алдыңыз ба?":
            fig = px.bar(df,
                         x="13.\тСіз қалай ойлайсыз, сіз қашықтықтан оқыту кезінде белсенді бола алдыңыз ба? / Как вы считате, Вы стали активнее при дистанционном обучении?",
