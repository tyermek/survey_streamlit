...
def plot_responses(df, plot_types):
    for plot_type in plot_types:
        if plot_type == "Үйде жұмыс орнының қолжетімділігі":
            ...
            table_data1.columns = ["Жауап", "Компьютерде күніне қанша сағат отырасыз?"]
            ...
            table_data2 = df.groupby(["16. Компьютерде күніне қанша сағат отырасыз?", "18. Қашықтықтан оқыту кезінде гимнастикалық жаттығуды Сіз күніне неше рет жасайсыз?"]).size().reset_index(name='Қашықтықтан оқыту кезінде гимнастикалық жаттығуды Сіз күніне неше рет жасайсыз?')
            ...
            table_data3.columns = ["Жауап", "Сізге онлайн сабақтарға қатысу ыңғайлы ма?"]
            ...
            table_data4 = df.groupby(["15. Үй жағдайында Сізге онлайн сабақтарға қатысу ыңғайлы ма?", "22. Сізге қалай сабақ оқу ыңғайлы?"]).size().reset_index(name='Сізге онлайн сабақтарға қатысу ыңғайлы ма? / Сізге қалай сабақ оқу ыңғайлы?')
            ...
            table_data.columns = ["Жауап", "Компьютер немесе гаджет салдарынан отбасы мүшелерімен жанжал туындайды ма?"]
            st.write(table_data.set_index('Жауап').T)
            ...
            table_data.columns = ["Жауап", "Мұғалімнің тапсырмаларын қалай жиі орындайсыз?"]
            st.write(table_data.set_index('Жауап').T)
            ...
            table_data.columns = ["Жауап", "Сіз қашықтықтан оқыту кезінде белсенді бола алдыңыз ба?"]
            st.write(table_data.set_index('Жауап').T)
