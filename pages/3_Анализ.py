...
def plot_responses(df, plot_types):
    for plot_type in plot_types:
        if plot_type == "Үйде жұмыс орнының қолжетімділігі":
            ...
            st.plotly_chart(fig)
            st.write(df[["Қашықтықтан оқыту кезінде сабақтарды орындау үшін Сіздің үйде жұмыс орны бар ма? "]])
            ...
            st.plotly_chart(fig)
            st.write(df[["16.\tКомпьютерде күніне қанша сағат отырасыз? / Сколько часов в день Вы сидите за компьютером?"]])
            ...
            fig.update_layout(xaxis_tickangle=-45)
            st.plotly_chart(fig)
            st.write(df[["15.\tҮй жағдайында Сізге онлайн сабақтарға қатысу ыңғайлы ма? / Удобно ли Вам в домашних условиях участвовать в онлайн уроках? "]])
            ...
            st.plotly_chart(fig)
            st.write(df[["27.\tКомпьютер немесе гаджет салдарынан отбасы мүшелерімен жанжал туындайды ма? / Возникают ли ссоры с членами семьи из-за компьютера или гаджета?"]])
            ...
            st.plotly_chart(fig)
            st.write(df[["10.\tМұғалімнің тапсырмаларын қалай жиі орындайсыз? / Как часто Вы выполняете задания учителя?"]])
            ...
            fig.update_layout(xaxis_tickangle=-45)
            st.plotly_chart(fig)
            st.write(df[["13.\tСіз қалай ойлайсыз, сіз қашықтықтан оқыту кезінде белсенді бола алдыңыз ба? / Как вы считате, Вы стали активнее при дистанционном обучении?"]])
