...
@st.cache_data
def load_data():
    return pd.read_excel("35950.xlsx",
                         sheet_name="35950")
...
def plot_responses(df):
    ...
    fig = px.pie(df, names="Қашықтықтан оқыту кезінде сабақтарды орындау үшін Сіздің үйде жұмыс орны бар ма?",
                 title="Cабақтарды орындау үшін Сіздің үйде жұмыс орны бар ма?")
    fig.update_layout(xaxis_tickangle=-45)
    st.plotly_chart(fig)

    fig = px.bar(df, x="16. Компьютерде күніне қанша сағат отырасыз? / Сколько часов в день Вы сидите за компьютером?",
    ...
    # Count occurrences for the bar chart
    response_counts = df[
        "13. Сіз қалай ойлайсыз, сіз қашықтықтан оқыту кезінде белсенді бола алдыңыз ба? / Как вы считате, Вы стали активнее при дистанционном обучении?"].value_counts().reset_index()
    response_counts.columns = ['Response', 'Count']

    fig = px.bar(response_counts, x='Response', y='Count',
